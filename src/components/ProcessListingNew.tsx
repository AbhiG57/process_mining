  ...
  return (
    <div className="min-h-screen bg-[#1A1D2B] text-white px-6 py-4">
      ...
      <div className="flex gap-4 mb-6">
        {/* Overview Card */}
        <div className="flex-[2] bg-[#222638] rounded-lg p-4">
          <h3 className="text-sm font-medium mb-4">Overview</h3>
          <div className="flex gap-16">
            <div>
              <div className="text-3xl font-bold mb-1">{totalProcesses}</div>
              <div className="text-sm text-gray-400">Total Processes</div>
            </div>
            <div className="flex-1">
              <div className="text-sm text-gray-400 mb-3">Breakdown by Status</div>
              <div className="space-y-2.5">
                {statusCards.map((card) => (
                  <div key={card.label} className="flex items-center gap-3">
                    <div className="w-28 text-sm text-gray-400">{card.label}</div>
                    ...
                    <div className="w-10 text-right text-sm text-gray-300">{card.value}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex-1 bg-[#222638] rounded-lg p-4">
      ...
      <div className="bg-[#222638] rounded-lg">
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 className="text-sm font-medium">All Processes</h2>
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="bg-[#222638] text-white text-sm px-7 py-1.5 rounded w-56 focus:outline-none"
              />
            </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                    activeFilter === filter 
                      ...
                      : 'bg-[#222638] text-white hover:bg-gray-600'
                  }`}
                >
                  {filter}
                </button>
        ...
        <div className="p-4 space-y-2">
          {filteredProcesses.map((process) => (
            <div
              key={process.id}
              className="flex items-center justify-between py-3 px-4 hover:bg-[#222638] transition-colors cursor-pointer rounded"
              onClick={() => navigate(`/process/${process.id}`)}
            >
                              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <div className="text-sm font-semibold">{process.title}</div>
                  <span
              ...
              <div className="flex items-center gap-2 ml-4">
                <button className="bg-[#222638] hover:bg-gray-600 px-3 py-1 rounded text-sm font-medium text-white transition-colors">
                  Test
                </button>
        ...
        <div className="flex items-center justify-center gap-1 mt-6">
          <button className="px-2 py-1 rounded text-sm text-gray-400 hover:text-white">‹</button>
          <button className="px-2 py-1 rounded text-sm bg-[#6B48FF] text-white">1</button>
          <button className="px-2 py-1 rounded text-sm text-white hover:bg-gray-700">2</button>
          <button className="px-2 py-1 rounded text-sm text-white hover:bg-gray-700">3</button>
          <span className="px-2 py-1 text-sm text-gray-400">...</span>
          <button className="px-2 py-1 rounded text-sm text-white hover:bg-gray-700">8</button>
          <button className="px-2 py-1 rounded text-sm text-gray-400 hover:text-white">›</button>
        </div>
      </div>
    </div>
