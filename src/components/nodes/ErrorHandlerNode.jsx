    ...
    <div className="rounded-lg bg-red-900 border-2 border-red-600 shadow-lg p-3 min-w-[140px] min-h-[60px] flex flex-col relative text-xs">
      ...
      <div className="flex items-center gap-2 mb-1 justify-between">
        <span className="flex items-center gap-1">
          ...
          <span className="font-bold text-red-100 text-xs">{data.label}</span>
        </span>
        <button className="text-slate-400 hover:text-red-400 text-xs p-1" onClick={data.onDelete}>
          <FontAwesomeIcon icon={faTrash} />
        </button>
      </div>
      <div className="flex-1 border-2 border-dashed border-red-400 rounded bg-red-950/60 p-1 text-[10px] text-red-200">
        {data.description}
      </div>
    </div>
  );
